...
#All the data sets for 2021 showed missing Crime ID values constituting about 14-20% of the column
#The crime id can be taken as the primary key as it is the unique identifier for a row
#If the crime id is unknown, the corresponding records might lose their significance,
  #potentially losing data integrity
#It also brings risk in the accuracy and reliability in future analysis
  #because the missing crime id could cause duplicated rows and thus, create incorrect trends
#Thus, the rows with missing crime id are removed
...
dim(bsCrimeRate2021)
View(bsCrimeRate2021)

colSums(is.na(bsCrimeRate2021))
#The column "Context" contains missing values that are equal to the number of rows in the data set
  #meaning that, the column is completely empty and useless
#Thus, it is removed
...
str(bsCrimeRate2021)
#The summary for columns "Longitude" and "Latitude" show a small difference between 3rd quartile and max outlier
#There seems to be little skewness in the data distribution
summary(bsCrimeRate2021)

#bsCrimeRate2021 contains duplicated rows that can cause inaccuracies while imputing missing values
